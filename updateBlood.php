		...
		if($errorMessage=="" && $errorMessage1== "" && $sidErr== "" && $bloodErr== "" && $dateErr == "")
			{
				if($db_found){
					...
					$SQL = "SELECT * FROM Blood WHERE SID = $sid AND Blood_GROUP=$blood";
					$result = mysqli_query($conn,$SQL);
					$num_rows = mysqli_num_rows($result);
					
					if ($num_rows > 0) {
						$SQL= "UPDATE Blood SET Last_Blood_Donation=$date WHERE SID=$sid && Blood_GROUP=$blood";
						$result = mysqli_query($conn,$SQL);
						mysqli_close($conn);
						header('Location: update.php');
							...
							echo "<h1>No Data Found in This Social Number.</h1>";
						}
				}				
			}
...
<div class="container">
                ...
                <ul class="nav navbar-nav">
                    ...
                    <li class="dropdown"><a href="#" data-toggle="dropdown">Blood Info <span class="caret"></span></a>
                        <ul class="dropdown-menu">
                            ...
                            <li class="active"><a href="updateBlood.php">Update Blood Info</a></li>
	...
	<div class="row">
		<div id = "heading" >
			<h2 style="color:blue"><strong>Update Blood Info</strong><h2>
		</div>
	</div>
		

		<form name="blood" method="POST" action="updateBlood.php">
			<div class="row">
				<table border="0" style="width:100%" class="signuptable">

					<tr>
					<th><label>Social ID:</label></th>
					<td><input Style="border-radius:7px;" type="number" name="sid" maxlength="8"/><span class="error"> <?php echo $errorMessage;?></span></td>
					</tr>


					<tr>
					<th><label>Blood Group:</label></th>
					<td><input Style="border-radius:7px;" type="text" name="blood" maxlength="3"/><span class="error"> <?php echo $errorMessage1;?></span></td>
					</tr>


					<tr>
					<th><label>Last Date Of Blood Donation:</label></th>
					<td><input Style="border-radius:7px;" type="date" name="blooddate"/><span class="error"> <?php echo $dateErr;?></span></td>
					</tr>
